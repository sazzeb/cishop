...
function submit_test()
{
    ...
    //simulate order creations
    $paypal_id = 88;
    $this->load->module('store_orders');
    $this->load->module('site_security');
    $this->load->module('store_basket');
    $customer_sesssion_id = $this->site_security->_decrypt_string($custom);
    $query = $this->store_basket->get_where_custom('session_id',$customer_sesssion_id);
    foreach ($query->result() as $row) {
    ...
    for ($i=0; $i < $num_orders ; $i++) { 
        $this->store_orders->_auto_generate_orders($paypal_id, $customer_sesssion_id);
...
function ipn_listener()
{
    ...
    foreach ($_POST as $key => $value) {
        if($key == 'custom')
        {
            $customer_sesssion_id = $this->site_security->_decrypt_string($value);
            $value = $customer_sesssion_id;
        }
    ...
    $max_id = $this->get_max;
    $this->load->module('store_orders');
    $this->store_orders->_auto_generate_orders($max_id, $customer_sesssion_id);
