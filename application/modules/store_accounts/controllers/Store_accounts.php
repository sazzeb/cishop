...
function _make_sure_delete_allowed($update_id)
{
    ...
    return FALSE;
    //Do not Delete if User has Item in basket or shopper track
    $mysql_query = "select * from basket where shopper_id = '$update_id'";
    ...
    if($num_rows>0)
    {
        ...
        $mysql_query = "select * from store_shoppertarck where shopper_id = '$update_id'";
        $query = $this->_custom_query($mysql_query);
        $num_rows = $this->num_rows($query);
        if($num_rows>0)
        {
...
function delete($update_id)
{
    ...
    if ($submit=="Cancel") {
        ...
        if ($allowed == FALSE)
        {
             $flash_msg = "User have item on basket or shopper track, you are not allowed to delete this account";
