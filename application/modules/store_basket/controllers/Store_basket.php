...
{
    /***
    NOTE: make sure there are no items on shoppertrack
    with shopper_id and session_id

    If there are items on shoppertarck with thesame session_id and shopper_id then 

    Regenerate the session_id for these user and update the session_id variable 
    **/
    ...
    if($num_rows>0)
    {
        //items conflicting with store shopper track
        //regenrate the session id

        session_regenerate_id();

        //update or change the data['session_id']
