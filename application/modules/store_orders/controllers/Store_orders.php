...
{
    //NOTE: notifies customer when an update id of this order has been updated
...
{
    //Note: This get called by order status and figure out the order status title
    $this->load->module('store_order_status');
...
function submit_order_status()
{
    ...
    if($submit == 'Cancel')
    {
        //get de current order status for dis order

        $query = $this->get_where($update_id);
        foreach ($query->result() as $row) {
            $order_status = $row->order_status;
        }
        $target_url = base_url().'store_order/browse/status'.$order_status;
        redirect('$target_url');
    }elseif($submit=='Submit'){
        $data['order_status'] = $order_status;
        $this->_update($update_id,$data);
        $this->_send_auto_notify($update_id); //notify customer of this update
...
function view() 
{
    ...
    $data['order_status'] = $order_status;
    $data['options'] = $this->store_order_status->_get_dropdown_option();
...
{
    //find out the totatal amount taken in from this order
...
function _get_shopper_id($customer_sesssion_id)
{
    $this->load->module('store_basket');
    $query = $this->store_basket->get_where_custom('shopper_id', $customer_sesssion_id);
    foreach ($query->result() as $row)
    {
        $shopper_id = $row->shopper_id;
    }
...
function _auto_generate_orders($paypal_id, $customer_sesssion_id)
{
    //get the IPN listener
    $this->load->module('site_security');
    ...
    $data['session_id'] = $customer_sesssion_id;
    $data['opened'] = 0;
    $data['order_status'] = 0;
    $data['shopper_id'] = $this->_get_shopper_id($customer_sesssion_id);
    $data['mc_gross'] = $this->_get_mc_gross($paypal_id);
    $this->_insert($data);
    //transer orders to store shoppers tracks
    $this->load->module('store_shoppertrack');
    $this->store_shoppertrack->_transfer_frombasbet($customer_sesssion_id);
}
